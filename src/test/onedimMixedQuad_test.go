...
func TestOneDimMixedQuad(t *testing.T) {
	...
	if !tree.IsOverlap(target0, 2) { // 303が存在するのでOverlab=trueが正解
		t.Error("target0")
	}

	target1 := tr.Indexs{0b111}      // 31
	if !tree.IsOverlap(target1, 3) { // 31が存在するのでOverlag=trueが正解
		t.Error("target1")
	}
	...
	if tree.IsOverlap(target3, 3) { // 302は存在しないのでOverlap=falseが正解
		t.Error("target3")
	}
	...
	target5 := tr.Indexs{0b111}      // 不正データ（3ビット目は無視）
	if !tree.IsOverlap(target5, 1) { // 3が存在するのでOverlag=trueが正解
		t.Error("target5")
	}
